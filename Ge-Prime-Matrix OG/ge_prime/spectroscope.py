"""Spektroskopie — SubstanceIndex + Intervall-Index (v40)."""
...
from ge_prime.hierarchy import (
    ...
    get_hierarchy,
    get_interval_index,
    nodes_for_token_span,
    token_char_map,
)
from ge_prime.substance_span import local_kgv, passes_kgv_gate
...
def spectroscope_analyze(
    ...
    *,
    ...
    s_target = 1
    for prime, exp in target_fp.exponents.items():
        s_target = math.lcm(s_target, prime**exp)

    h = get_hierarchy(document)
    idx = get_interval_index(document)
    affected = nodes_for_token_span(h, token_start, token_count, interval_index=idx)

    target_sentence_curve = []
    for s in affected["sentences"]:
        target_sentence_curve.extend(
            [p for p in extract_sentence_curve(document) if p["token_start"] == s.token_start]
        )
    target_line_curve = []
    ...
    *,
    ...
    for idx, cs, ce in mapping:
        if ce > selection_start and cs < selection_end:
            t_start = min(t_start, idx) if t_start == 0 and idx == 0 else min(t_start, idx)
            t_end = max(t_end, idx + 1)
    if mapping:
        selected = [
            idx
            for idx, cs, ce in mapping
