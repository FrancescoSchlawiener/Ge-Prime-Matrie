...
def _token_window_literal_ratio(curve_a: list[dict], curve_b: list[dict]) -> tuple[float, int]:
    if not any("normalized" in p for p in curve_a) or not any("normalized" in p for p in curve_b):
        return 0.0, 0
    tokens_a = [p["normalized"] for p in curve_a]
    tokens_b = [p["normalized"] for p in curve_b]
...
def _compute_literal_metrics(
    ...
    *,
    ...
    emit_metric_debug(
        "literal_intersections",
        {
            "aligned": aligned,
            "length_a": len_a,
            "length_b": len_b,
            "positional_ratio": positional_ratio,
