...
from ge_prime.multiset_geom import perm_fits_width, perm_index, perm_space
from gpm.cell_geom import (
    ...
    _category_key,
    build_cell_geometry,
    cell_geometry_encodable,
    split_for_perm_overflow,
)
from gpm.model import GpmDocument, GpmToken
from gpm.boundary_suffix import boundary_suffix_for_token
...
@dataclass
class HierarchyNode:
    layer: str  # semantic | structural
    level: str  # phrase | sentence | paragraph | line | page
    token_start: int
    token_count: int
    perm_index: int = 0
    perm_space: int = 1
    ...
    boundary_suffix: int = 0  # v7: 0=none, 1='.', 2='!', 3='?'
...
def nodes_intersecting(nodes: list[HierarchyNode], span: TokenSpan) -> list[HierarchyNode]:
    return [
        n
        for n in nodes
...
def gap_starts_page(gap: str) -> bool:
    """Formfeed (\\f) — nur für Export/PDF-Seitenumbrüche, nicht Standard-Hierarchie."""
    return "\f" in gap
...
def split_page_spans(line_spans: list[TokenSpan], gaps: list[str]) -> list[TokenSpan]:
    """Token-Spans pro Druckseite (\\f). Nur für Export/PDF — nicht in build_document_hierarchy."""
    if not line_spans:
...
def _category_keys_for_span(
    ...
    from gpm.cell_geom import _category_key
...
def _build_word_geometry(
    ...
    keys = _category_keys_for_span(
        document, TokenSpan(span.token_start, mid), explicit_map
...
def _sentence_nodes(
    document: GpmDocument,
    phrases: list[HierarchyNode],
) -> list[HierarchyNode]:
    nodes: list[HierarchyNode] = []
    for span in split_sentence_spans(len(document.tokens), document.gaps):
        phrase_keys = [
            PhraseCategoryKey(
                s_phrase=_phrase_substance(
                    document, TokenSpan(p.token_start, p.token_count)
                ),
...
def _page_nodes(lines: list[HierarchyNode], document: GpmDocument) -> list[HierarchyNode]:
    """Intern — siehe build_page_nodes_for_export()."""
    line_spans = [TokenSpan(l.token_start, l.token_count) for l in lines]
    pages: list[HierarchyNode] = []
    for p_idx, span in enumerate(split_page_spans(line_spans, document.gaps)):
        line_keys = [
            LineCategoryKey(s_line=line.s_level, token_count=line.token_count)
            for line in lines
            if intervals_overlap(line.token_start, line.token_count, span.token_start, span.token_count)
        ]
        if not line_keys:
            line_keys = [LineCategoryKey(s_line=_phrase_substance(document, span), token_count=span.token_count)]
        freqs, _seq, n, i_val = _geometry_from_keys(line_keys, lambda k: k)
        pages.append(
            HierarchyNode(
                layer="structural",
                level="page",
...
def build_page_nodes_for_export(document: GpmDocument) -> list[HierarchyNode]:
    """Page-Nodes aus Zeilen + Formfeed-Gaps — nur Export/PDF, nicht I-Kurve oder Standard-Hierarchie."""
    explicit_map = dict(document.explicit)
    lines = _line_nodes(document, explicit_map)
    return _page_nodes(lines, document)


def build_semantic_encodable_segments(document: GpmDocument) -> list[CellGeometry]:
    """Encodable Phrase/Satz-Overflow-Segmente für v6 semantic body."""
    explicit_map = dict(document.explicit)
