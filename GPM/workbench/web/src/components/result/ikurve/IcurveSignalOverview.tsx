...
export function IcurveSignalOverview({ data }: IcurveSignalOverviewProps) {
  ...
  const substCmp = (data.substance_comparison ?? comparison.substance_geometry ?? {}) as Record<string, unknown>;
  const relCmp = (data.relation_comparison ?? {}) as Record<string, unknown>;

  const segments = [
    { label: t("ikurve.signals.wordGeometry"), score: Number(comparison.geometry_score_dtw ?? 0) },
    { label: t("ikurve.signals.fusionScore"), score: Number(comparison.geometry_score ?? 0) },
    {
      label: t("ikurve.signals.substanceGgt"),
      score: Number(substCmp.geometry_score ?? 0),
    },
    {
      label: t("ikurve.signals.cellLine"),
      score: Number(cellCmp.geometry_score ?? 0),
    },
    {
      label: t("ikurve.signals.relationTopology"),
      score: Number(relCmp.relation_score ?? 0),
    },
