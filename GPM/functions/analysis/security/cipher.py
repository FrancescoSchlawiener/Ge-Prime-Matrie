...
def parse_key_token(raw: str) -> KeyEntry:
    ...
    normalized = prepare_substrate(raw, AlphabetProfile.OG)
...
def _key_base_int(key: KeyEntry) -> int:
    if key.kind == "word":
        substance, _ = encode_si(key.value, AlphabetProfile.OG)
        return substance
...
def encrypt_text(
    ...
    *,
    mode: str,
    keys_raw: str | list[str],
    profile: AlphabetProfile = AlphabetProfile.OG,
) -> dict:
    ...
    document, stats = compile_text(text, profile)
    if stats.word_count > MAX_CIPHER_TOKENS:
        ...
        header_out.append(
            {
                ...
                "word_original": entry.word_canonical,
    ...
    gaps_enc = [_encrypt_gap(gap, keys, i * 97) for i, gap in enumerate(document.gaps)]
...
def _document_from_payload(payload: dict, keys: list[KeyEntry], *, mode: str, profile) -> GpmDocument:
    dual = _dual_lane(mode)
    header = []
    for row in payload["header"]:
        ...
        header.append(
            GpmHeaderEntry(
                word_id=row["word_id"],
                word_canonical=row["word_original"],
    ...
    for pos, row in enumerate(payload["tokens"]):
        key = keys[pos % len(keys)]
        perm_index = _decrypt_i(row["i_enc"], row["perm_space"], key, pos, dual_lane=dual)
        tokens.append(
            GpmToken(
                word_id=row["word_id"],
    ...
    gaps = [_decrypt_gap(gap_enc, keys, i * 97) for i, gap_enc in enumerate(payload["gaps_enc"])]
...
def decrypt_ciphertext(
    ...
    *,
    ...
    keys = parse_keys(keys_raw, mode=mode)
    document = _document_from_payload(payload, keys, mode=mode, profile=profile)
...
def demo_roundtrip(word: str, key: str, mode: str = MODE_WORD) -> dict:
    ...
    substance, perm_index = encode_si(prepare_substrate(word, AlphabetProfile.OG), AlphabetProfile.OG)
    return {
        "mode": mode,
