"""Zell-Geometrie (I_Satz) — profil-aware Port."""
...
from analysis.case.codes import CASE_EXPLICIT
from analysis.document.model import GpmDocument, GpmToken
from perm.multiset import perm_decode, perm_fits_width, perm_index, perm_space
...
def build_cell_geometry(
    ...
    *,
    ...
    if len(tokens) > MAX_CELL_TOKENS:
        raise ValueError(f"Zelle hat {len(tokens)} Token — Maximum ist {MAX_CELL_TOKENS}.")
...
def split_into_slices(token_count: int, gaps: list[str]) -> list[CellSlice]:
    if token_count == 0:
        return []
...
def split_for_perm_overflow(
    ...
    *,
    ...
    if not tokens:
        return []
    try:
        geom = build_cell_geometry(tokens, token_start=token_start, explicit_map=explicit_map)
    except ValueError:
        if len(tokens) <= 1:
            raise
        mid = len(tokens) // 2
        return split_for_perm_overflow(
            tokens[:mid], token_start=token_start, explicit_map=explicit_map
        ) + split_for_perm_overflow(
            tokens[mid:], token_start=token_start + mid, explicit_map=explicit_map
        )
